...
fn process_input(input: &str, conn: &mut Connection, history: &Vec<String>) -> bool {
    ...
    match &*input_low {
        ...
        ":snake" => {
            ...
            println!(
                "Snakes bred under MIT License by Johannes Schickling
        ...
        ":insult" => {
            ...
            println!(
                "Fleet build under MIT License by Johannes Schickling
                    via github /schickling/rust-examples/"
            );
    ...
    for i in 0..(cols.len()) {
        if table.get_col_name(i).unwrap().len() > 27 {
            ...
            print!(
                "| {1: ^0$} ",
                min(30, cols[i]),
                table.get_col_name(i).unwrap_or("none")
            );
        }
    }
    println!("|");
...
fn display_meta(table: &mut DataSet) {
    ...
    for i in 0..table.get_col_cnt() {
        cols.push(max(
            12,
            max(
                table.get_col_name(i).unwrap().len(),
